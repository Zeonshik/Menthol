...
//                        cursor = null;
    ...
    private void markMessageSavedDeletedInternal(String tableName, long dialogId, int messageId, TLRPC.Message message, NativeByteBuffer customParams) throws Exception {
        SQLitePreparedStatement state = null;
        NativeByteBuffer newCustomParams = null;
        try {
            if (customParams != null) {
                MessageCustomParamsHelper.readLocalParams(message, customParams);
            }
            message.savedDeleted = true;
            newCustomParams = MessageCustomParamsHelper.writeLocalParams(message);
            state = database.executeFast("UPDATE " + tableName + " SET custom_params = ? WHERE mid = ? AND uid = ?");
            state.requery();
            if (newCustomParams != null) {
                state.bindByteBuffer(1, newCustomParams);
            } else {
                state.bindNull(1);
            }
            state.bindInteger(2, messageId);
    ...
    private ArrayList<Long> markMessagesAsDeletedInternal(long dialogId, ArrayList<Integer> messages, boolean deleteFiles, int mode, int threadMessageId, boolean preserveDeletedMessages) {
        SQLiteCursor cursor = null;
        SQLitePreparedStatement state = null;
        try {
...
//                }
                ...
                if (dialogId != 0) {
                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT uid, data, read_state, out, mention, mid, custom_params FROM messages_v2 WHERE mid IN(%s) AND uid = %d", ids, dialogId));
                } else {
                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT uid, data, read_state, out, mention, mid, custom_params FROM messages_v2 WHERE mid IN(%s) AND is_channel = 0", ids));
                }
                ...
                try {
                    while (cursor.next()) {
                        ...
                        boolean keepDeletedMessage = preserveDeletedMessages && did != currentUser && !DialogObject.isEncryptedDialog(did);
                        unknownMessages.remove((Integer) mid);
                        if (!keepDeletedMessage) {
                        ...
                        if (!keepDeletedMessage && !DialogObject.isEncryptedDialog(did) && !deleteFiles && did != currentUser) {
                            continue;
                        }
                        NativeByteBuffer data = cursor.byteBufferValue(1);
                        if (data != null) {
                            ...
                            if (keepDeletedMessage) {
                                markMessageSavedDeletedInternal("messages_v2", did, mid, message, cursor.byteBufferValue(6));
                            } else if (deletedMessages != null) {
                                deletedMessages.add(message);
                            }
                            data.reuse();
                            if (!keepDeletedMessage && (DialogObject.isEncryptedDialog(did) || deleteFiles)) {
                                addFilesToDelete(message, filesToDelete, idsToDelete, namesToDelete, false);
                            }
                ...
                if (dialogId < 0) {
                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT uid, data, read_state, out, mention, mid, custom_params FROM messages_topics WHERE mid IN(%s) AND uid = %d", ids, dialogId));
                    ...
                    try {
                        ...
                        if (data != null) {
                            ...
                            data.reuse();
                            if (keepDeletedMessage) {
                                markMessageSavedDeletedInternal("messages_topics", did, mid, message, cursor.byteBufferValue(6));
                            } else {
                                addFilesToDelete(message, filesToDelete, idsToDelete, namesToDelete, false);
                            }
                            if (message.action instanceof TLRPC.TL_messageActionTopicCreate) {
    ...
    public ArrayList<Long> markMessagesAsDeleted(long dialogId, ArrayList<Integer> messages, boolean useQueue, boolean deleteFiles, int mode, int topicId, boolean preserveDeletedMessages) {
        if (messages.isEmpty()) {
            return null;
        }
        if (useQueue) {
            storageQueue.postRunnable(() -> markMessagesAsDeletedInternal(dialogId, messages, deleteFiles, mode, topicId, preserveDeletedMessages));
        } else {
            return markMessagesAsDeletedInternal(dialogId, messages, deleteFiles, mode, topicId, preserveDeletedMessages);
        }
        return null;
    }

    private ArrayList<Long> markMessagesAsDeletedInternal(long channelId, int mid, boolean deleteFiles) {
        return markMessagesAsDeletedInternal(channelId, mid, deleteFiles, false);
    }

    private ArrayList<Long> markMessagesAsDeletedInternal(long channelId, int mid, boolean deleteFiles, boolean preserveDeletedMessages) {
        SQLiteCursor cursor = null;
        SQLitePreparedStatement state = null;
        try {
            ArrayList<Long> dialogsIds = new ArrayList<>();
            ...
            cursor = database.queryFinalized(String.format(Locale.US, "SELECT uid, data, read_state, out, mention, mid, custom_params FROM messages_v2 WHERE uid = %d AND mid <= %d", -channelId, mid));

            try {
                while (cursor.next()) {
                    long did = cursor.longValue(0);
                    if (preserveDeletedMessages && !dialogsIds.contains(did)) {
                        dialogsIds.add(did);
                    }
                    if (did != currentUser) {
                        int read_state = cursor.intValue(2);
                    ...
                    if (data != null) {
                        TLRPC.Message message = TLRPC.Message.TLdeserialize(data, data.readInt32(false), false);
                        message.readAttachPath(data, getUserConfig().clientUserId);
                        if (preserveDeletedMessages && did != currentUser && !DialogObject.isEncryptedDialog(did)) {
                            markMessageSavedDeletedInternal("messages_v2", did, cursor.intValue(5), message, cursor.byteBufferValue(6));
                        } else {
                            addFilesToDelete(message, filesToDelete, idsToDelete, namesToDelete, false);
                        }
                        data.reuse();
    ...
    public ArrayList<Long> markMessagesAsDeleted(long channelId, int mid, boolean useQueue, boolean deleteFiles) {
        return markMessagesAsDeleted(channelId, mid, useQueue, deleteFiles, false);
    }

    public ArrayList<Long> markMessagesAsDeleted(long channelId, int mid, boolean useQueue, boolean deleteFiles, boolean preserveDeletedMessages) {
        if (useQueue) {
            storageQueue.postRunnable(() -> markMessagesAsDeletedInternal(channelId, mid, deleteFiles, preserveDeletedMessages));
        } else {
            return markMessagesAsDeletedInternal(channelId, mid, deleteFiles, preserveDeletedMessages);
        }
        return null;
    }
