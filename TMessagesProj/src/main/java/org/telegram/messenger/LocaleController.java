...
public class LocaleController {
    ...
    public LocaleController() {
        ...
        if (BuildVars.USE_CLOUD_STRINGS) {
            for (int i = 0; i < languages.size(); i++) {
                LocaleInfo info = languages.get(i);
                if (info.builtIn && !"en".equals(info.shortName)) {
                    info.pathToFile = "remote";
                }
            }
        }
    ...
    private String getStringInternal(String key, String fallback, int fallbackRes, int res) {
        String forcedValue = getForcedBrandlessString(key);
        if (forcedValue != null) {
            return forcedValue;
        }
        boolean forceLocal = shouldUseLocalBrandlessString(key);
        ...
        return replaceAppName(key, value);
    }

    private String replaceAppName(String key, String value) {
        if (TextUtils.isEmpty(value)) {
            return value;
        }
        if ((!TextUtils.isEmpty(key) && key.toLowerCase(Locale.US).endsWith("url")) || value.contains("://")) {
            return value;
        }
        value = replaceBrandlessTerms(value);
        if (value.indexOf("Telegram") < 0 && value.indexOf("TELEGRAM") < 0 && value.indexOf("Телеграм") < 0 && value.indexOf("ТЕЛЕГРАМ") < 0 && value.indexOf("телеграм") < 0) {
            return value;
        }
        String appName;
        try {
            appName = ApplicationLoader.applicationContext.getString(R.string.AppName);
        } catch (Exception e) {
            appName = null;
        }
        if (TextUtils.isEmpty(appName) || "Telegram".equals(appName)) {
            return value;
        }
        return value
                .replace("TELEGRAM", appName.toUpperCase(Locale.US))
                .replace("Telegram", appName)
                .replace("ТЕЛЕГРАМ", appName.toUpperCase(Locale.US))
                .replace("Телеграм", appName)
                .replace("телеграм", appName);
    }

    private boolean shouldUseLocalBrandlessString(String key) {
        return "SentAppCode".equals(key) || "SentAppCodeWithPhone".equals(key) || "SentAppCodeTitle".equals(key);
    }

    private String getForcedBrandlessString(String key) {
        if (!shouldUseLocalBrandlessString(key)) {
            return null;
        }
        boolean ru = currentLocale != null && "ru".equals(currentLocale.getLanguage());
        if ("SentAppCode".equals(key)) {
            return ru ? "Мы отправили код на другое Ваше устройство." : "We've sent the code to your other device.";
        } else if ("SentAppCodeWithPhone".equals(key)) {
            return ru ? "Мы отправили код на другое устройство, где авторизован %1$s." : "We've sent the code for %1$s to your other device.";
        } else if ("SentAppCodeTitle".equals(key)) {
            return ru ? "Проверьте сообщения" : "Check your messages";
        }
        return null;
    }

    private String replaceBrandlessTerms(String value) {
        return value
                .replace("**Telegram Premium**", "**Premium**")
                .replace("**Menthol Premium**", "**Premium**")
                .replace("*Telegram Premium*", "*Premium*")
                .replace("*Menthol Premium*", "*Premium*")
                .replace("__Telegram Premium.__", "__Premium.__")
                .replace("__Menthol Premium.__", "__Premium.__")
                .replace("TELEGRAM PREMIUM", "PREMIUM")
                .replace("MENTHOL PREMIUM", "PREMIUM")
                .replace("Telegram Premium", "Premium")
                .replace("Menthol Premium", "Premium")
                .replace("TELEGRAM BUSINESS", "BUSINESS")
                .replace("MENTHOL BUSINESS", "BUSINESS")
                .replace("Telegram Business", "Business")
                .replace("Menthol Business", "Business")
                .replace("TELEGRAM STARS", "STARS")
                .replace("MENTHOL STARS", "STARS")
                .replace("Telegram Stars", "Stars")
                .replace("Menthol Stars", "Stars")
                .replace("Telegram FAQ", "FAQ")
                .replace("Menthol FAQ", "FAQ")
                .replace("Telegram Features", "Features")
                .replace("Menthol Features", "Features")
                .replace("TELEGRAM SETTINGS", "SETTINGS")
                .replace("MENTHOL SETTINGS", "SETTINGS")
                .replace("Telegram Settings", "Settings")
                .replace("Menthol Settings", "Settings")
                .replace("Звёзды Telegram", "Звёзды")
                .replace("Звёзды Menthol", "Звёзды")
                .replace("Звезды Telegram", "Звезды")
                .replace("Звезды Menthol", "Звезды")
                .replace("Telegram для бизнеса", "Для бизнеса")
                .replace("Menthol для бизнеса", "Для бизнеса")
                .replace("Вопросы о Telegram", "Вопросы")
                .replace("Вопросы о Menthol", "Вопросы")
                .replace("Возможности Telegram", "Возможности")
                .replace("Возможности Menthol", "Возможности")
                .replace("Настройки Telegram", "Настройки")
                .replace("Настройки Menthol", "Настройки")
                .replace("настройках Telegram", "настройках")
                .replace("настройках Menthol", "настройках");
    }
    ...
    public static String getServerString(String key) {
        ...
        return getInstance().replaceAppName(key, value);
    }
    ...
    public static String formatPluralStringComma(String key, int plural, char symbol, Object... args) {
        try {
            ...
            value = getInstance().replaceAppName(key, value);
    ...
    public static String formatString(String key, String fallback, int res, int fallbackRes, Object... args) {
        try {
            String forcedValue = getInstance().getForcedBrandlessString(key);
            if (forcedValue != null) {
                return String.format(getInstance().currentLocale != null ? getInstance().currentLocale : Locale.US, forcedValue, args);
            }
            boolean forceLocal = getInstance().shouldUseLocalBrandlessString(key);
    ...
    public static CharSequence formatSpannable(String key, String fallback, int res, int fallbackRes, Object... args) {
        try {
            String forcedValue = getInstance().getForcedBrandlessString(key);
            if (forcedValue != null) {
                return new SpannableStringBuilder(String.format(getInstance().currentLocale != null ? getInstance().currentLocale : Locale.US, forcedValue, args));
            }
            boolean forceLocal = getInstance().shouldUseLocalBrandlessString(key);
            String value = !forceLocal && BuildVars.USE_CLOUD_STRINGS ? getInstance().localeValues.get(key) : null;
            if (value == null) {
