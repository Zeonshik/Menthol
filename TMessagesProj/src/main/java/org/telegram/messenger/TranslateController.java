...
//                messageObject.type == MessageObject.TYPE_ROUND_VIDEO ||
    ...
    public void translateMessageManually(MessageObject messageObject) {
        if (messageObject == null || messageObject.messageOwner == null || messageObject.getId() < 0 || messageObject.translating || !isTranslatable(messageObject, true)) {
            return;
        }
        final long dialogId = messageObject.getDialogId();
        final String language = getDialogTranslateTo(dialogId);
        messageObject.manually = true;
        ...
        if (MessageObject.getMedia(messageObject) instanceof TLRPC.TL_messageMediaPoll) {
            pushPollToTranslate(messageObject, language, (id, poll, lang) -> {
                if (messageObject.getId() != id) {
                    FileLog.e("manual translate asked " + messageObject.getId() + " poll but got " + id);
                }
                messageObject.translating = false;
                if (poll == null) {
                    messageObject.manually = false;
                ...
                messageObject.messageOwner.translatedToLanguage = lang;
            ...
            pushToTranslate(messageObject, language, (isTranscription, id, text, lang) -> {
                if (messageObject.getId() != id) {
                    FileLog.e("manual translate asked " + messageObject.getId() + " but got " + id);
                }
                messageObject.translating = false;
                if (text == null) {
                    messageObject.manually = false;
                ...
                messageObject.messageOwner.translatedToLanguage = lang;
                if (isTranscription) {
                    messageObject.messageOwner.translatedVoiceTranscription = text;
                } else {
