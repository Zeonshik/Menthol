...
public class FileLog {
    ...
    public static void dumpResponseAndRequest(int account, TLObject request, TLObject response, TLRPC.TL_error error, long requestMsgId, long startRequestTimeInMillis, int requestToken) {
        if (!isDetailedNetworkLogsEnabled() || !BuildVars.DEBUG_PRIVATE_VERSION || !BuildVars.LOGS_ENABLED || request == null) {
            return;
        }
    ...
    private static boolean isDetailedNetworkLogsEnabled() {
        return false;
    }

    public static void dumpUnparsedMessage(TLObject message, long messageId, int account) {
        if (!isDetailedNetworkLogsEnabled() || !BuildVars.DEBUG_PRIVATE_VERSION || !BuildVars.LOGS_ENABLED || message == null) {
            return;
        }
        try {
            checkGson();
    ...
    public void init() {
        ...
        // Disabled: thread/heap dumps caused long freezes on low-end devices.
        initied = true;
    }
    ...
    private void dumpANR() {
        ...
        // Heap dumps are too expensive on low-end devices and make ANRs worse.
    }
