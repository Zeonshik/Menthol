...
public class RestrictedLanguagesSelectActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public static HashSet<String> getRestrictedLanguages() {
        ...
        } else if (!MessagesController.getGlobalMainSettings().getBoolean("translate_button_restricted_languages_changed", false)) {
            String currentLanguage = LocaleController.getInstance().getCurrentLocaleInfo().pluralLangCode;
            if (currentLanguage != null && (restrictedLanguages.size() != 1 || !restrictedLanguages.contains(currentLanguage))) {
                restrictedLanguages = Sets.newHashSet(currentLanguage);
                MessagesController.getGlobalMainSettings().edit().putStringSet("translate_button_restricted_languages", restrictedLanguages).apply();
            }
        }
